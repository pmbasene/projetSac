...
def split_string(string, separator):
    ...
    ! Note!:  separator is not mandatory and do not be a None value'''
...
def openFile_changeString(file, s ,old_string, new_string):
    '''This function allows in first to open file with 
    writing mode and secondly to change an old string '''
    with open(file, 'w') as f:
        print('  > Changement de la variable [{old_string}] par son path  {new_string} dans le fichier {file}'.format(**locals()))
        # s = f.read()   # str type
...
PATH="/Users/ganasene/Desktop/doc/temp2"          # Preciser le path d'acces au repertoire ou se trouve les fichiers a traiter  
...
compteurFichier = 0                                # permet de compter le nombre de fichier traitei
for file in os.listdir(getdir):                    # parcourir les fichiers log du dossier precise en PATH
    ...
    with open(file, 'r') as f:       
        ...
        # Tester s'il existe des variable-repertoires dans le fichier en cours  
        if not matchVarPath:
            print('Il y a de variables dans ce fichier')
        else:
            print(" - Les variables sont existantes sont {matchVarPath} ".format(**locals()))
            print('   Remplacement des variables par leur path-repertoire correspondant:')   
        ...
        for varOld in matchVarPath:
            ...
            for item in matchParRep:                                     # each item will be a string. so we can use the split_string function
                ...
                if leftPartRep == var:
                    with open(file, 'w') as f:
                        openFile_changeString(file, s, varOld, rightpartRep)
